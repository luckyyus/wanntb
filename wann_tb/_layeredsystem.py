...
class LayeredHamiltonian:
    ...
    def __init__(self, tb_file='wannier90_tb.dat', lspinors=True):
        ...
        with open(tb_file, 'r') as f:
            ...
            self.k_lattice = np.linalg.inv(self.real_lattice) * TwoPi
            self.num_wann = int(f.readline())
            self.n_rpts = int(f.readline())
            # degenerate rpt
            ndegen = []
            while len(ndegen) < self.n_rpts:
                ndegen += f.readline().split()
            self.n_degen = np.array(ndegen, dtype=int)
            # initialize R_vec and Ham_R
            irpt = []
            self.ham_R = np.zeros((self.n_rpts, self.num_wann, self.num_wann), dtype=complex)
            ...
            for ir in range(self.n_rpts):
                ...
                self.ham_R[ir, :, :] = (hh[0, :, :] + 1j * hh[1, :, :])
            self.r_vec = np.array(irpt, dtype=float)  # 为了和k.R正常一些
            print('shape of ham_R is %s' % list(self.ham_R.shape))
            print('shape of R_vec is %s' % list(self.r_vec.shape))
