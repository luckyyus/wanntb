...
def get_occ_dos_kpar(ham_R, R_vec, R_vec_cart_T, kpts, efs, eta):
    nkpts = kpts.shape[0]
    n_ef = efs.shape[0]
    occ_ef_k = np.zeros((n_ef, nkpts), dtype=np.float64)
    dos_ef_k = np.zeros((n_ef, nkpts), dtype=np.float64)
    for ik in prange(nkpts):
        ...
        for i in range(n_ef):
            ...
            dos = dos_fermi(eig, ef, eta)
            occ_ef_k[i, ik] = np.sum(occ)
            dos_ef_k[i, ik] = np.sum(dos)
    return np.sum(occ_ef_k, axis=1) / nkpts, np.sum(dos_ef_k, axis=1) / nkpts


@njit(parallel=True, nogil=True)
def get_occ_dos_proj_kpar(ham_R, R_vec, R_vec_cart_T, num_wann, kpts, efs, eta):
    nkpts = kpts.shape[0]
    n_ef = efs.shape[0]
    occ_ef_k = np.zeros((n_ef, num_wann, nkpts), dtype=np.float64)
    dos_ef_k = np.zeros((n_ef, num_wann, nkpts), dtype=np.float64)
    for ik in prange(nkpts):
        kpt = kpts[ik]
        # uu[iproj, iband]
        ...
        for i in range(n_ef):
            ...
            dos_ef_k[i, :, ik] = np.sum(dos_proj, axis=1)
    return np.sum(occ_ef_k, axis=2) / nkpts, np.sum(dos_ef_k, axis=2) / nkpts