...
from .utility import fourier_phase_R_to_k, fourier_R_to_k, fourier_R_to_k_vec3, unitary_trans, occ_fermi, \
    fourier_R_to_k_curl, unitary_trans_sub, _ham_k_system, inv_e_d_r, inv_e_d_c, get_deltaU
...
@njit(nogil=True)
def _get_f_spin_omega(vh_b, js_a, inv2, f, num_wann):
...
def _get_shc_f_efs_k(ham_R, r_mat_R, R_vec, ss_R, R_cartT,
                     num_wann, kpt, efs, eta, xyz, mode, subwf):
    n_ef = efs.shape[0]
    if mode == 1:
    ...
    for i in range(n_ef):
        ...
        if mode == 1:
            ...
            ofg_k[i, :, :] = _get_f_spins_omega(js_a, Ah_b, f, num_wann)
...
def get_shc_kpar_fermi(ham_R, r_mat_R, R_vec, ss_R, R_cartT,
                       num_wann, kpts, efs, eta, xyz, subwf=None, mode=0):
    fac = - TwoPi
    nkpts = kpts.shape[0]
    n_ef = efs.shape[0]
