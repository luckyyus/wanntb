...
def get_delta_E(eig,ef,tau):
    return tau/((ef-eig)**2 + tau*tau/4) /(TwoPi)
...
def get_REtensors_kpar_kmesh(ham_R,r_mat_R, R_vec, _R_cartT, ss_R,num_wann, kpts,ef_list,tau,mode):
    ...
    for ik in prange(nkpts):
        ...
        if mode== 10:
            ...
            REE[:,:,1,ik] = get_orb_chi2_REtensors_kpath(ham_R,r_mat_R,R_vec, _R_cartT, num_wann, kpt,ef_list,tau)
...
def get_orb_REtensors_kpath(ham_R,r_mat_R, R_vec, _R_cartT, num_wann, kpt,ef_list,tau):
    ...
    for I in range(num_ef):
        ...
        for i in range(3):
            for m_ in range(num_wann):
                for n_ in range(m_ +1):
                    if f_ij[m_,n_]!=0 or m_==n_:
                        fo_k[i, m_, n_] = 1j*np.sum(g *(Ah_k[I_A[i], m_, :] * Ah_k[I_B[i], :, n_]
                                              - Ah_k[I_A[i], :, n_] * Ah_k[I_B[i], m_, :]) )
                        morb1_1[i,m_,n_] = -1j*(deltaU[I_A[i],:,m_].conj().T @operator @deltaU[I_B[i],:,n_] - deltaU[I_B[i],:,m_].conj().T@ operator @ deltaU[I_A[i],:,n_])
                        if m_!=n_:
                            fo_k[i,n_,m_] = fo_k[i, m_, n_].conjugate()
                            morb1_1[i,n_,m_] = morb1_1[i, m_, n_].conjugate()
...
@njit(nogil=True)
def get_orb_chi2_REtensors_kpath(ham_R,r_mat_R, R_vec, _R_cartT, num_wann, kpt,ef_list,tau):
    factor = 0.262
    num_ef =ef_list.shape[0]
    index1 = (0,0,0,1,1,1,2,2,2)
    index2 = (0,1,2,0,1,2,0,1,2)
            ...
            for m_ in range(num_wann):
                for n_ in range(m_ +1):
                    if  m_==n_:
                        fo_k[i, m_, n_] = 1j*np.sum(g *(Ah_k[I_A[i], m_, :] * Ah_k[I_B[i], :, n_]
                                              - Ah_k[I_A[i], :, n_] * Ah_k[I_B[i], m_, :]) )
                        morb1_1[i,m_,n_] = 2*(deltaU[I_A[i],:,m_].conj().T @operator @deltaU[I_B[i],:,n_]).imag
